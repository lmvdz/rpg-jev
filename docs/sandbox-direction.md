...
**Read and not yet built: heat.** Going through `heat.ts` against the schema, a two-party act needs six things the kernel does not have: two things in one env (`src.`, `tgt.`) with effects that write to either; the state as it was when the rule began (`was.`), for the surface's lead and for thermal shock; an `exp` operator; properties worked out again after the state has changed (ignition reads flammability after drying); `put` of a small literal object (what is burning, and its fuel); and a note per alternative, since one act says several things. Conservation there is arithmetic today (what the target gains is taken from the source by their capacities). It wants to be a fifth kind of effect, an `exchange` between two quantities by their capacities, which conserves by construction. None of the six is a function in a row.

**So far.** Rules that act on one thing over time can be data, exactly, and the kernel that reads them (`expr.ts` and `rules.ts`) is under 300 lines that do not grow with the rules, with as many again in `compile.ts` to make them fast. What is not yet shown: two-party acts, the minds' tables (salience is still typed, and should be fitted from Jev's logged choices), whether a generative model can write such rows and Jev ratify them, and whether rows written that way move held-out derivability, which is the only number that answers the problem this section opens with.
