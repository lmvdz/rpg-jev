...
- **In play the mouse picks glyphs by their own pixels** (`play/pick-glyph.ts`); the editor still picks tiles. A tall glyph's crown is drawn over the tiles behind it, so picking by tile pointed past the tree at the grass. The ray's walk across the grid now tells of every tile it passes over, and each glyph standing within two tiles of that path is tested as the shader draws it: the shader places pixel (x, y) of a glyph at A + xB + yC in clip space, the mouse fixes two ratios, and that leaves two linear equations for the pixel under the mouse, which is looked up in the font with the outline counted as part of the glyph. The nearest glyph hit wins, and the ray stops at the ground it lands on, so a glyph behind a wall is not reached. It allocates nothing, since it runs every frame. Sway and an actor's motion are not followed. Measured over the woods: a tree is named at 142 of 252 sample pixels where picking by tile named one at 66.
