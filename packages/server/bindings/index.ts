...
import ConfigureJepaReducer from "./configure_jepa_reducer.ts";
import JoinReducer from "./join_reducer.ts";
import ObserveReducer from "./observe_reducer.ts";
import PopulateReducer from "./populate_reducer.ts";
...
import JepaTickRow from "./jepa_tick_table.ts";
import ViewerRow from "./viewer_table.ts";
...
const tablesSchema = __schema({
  event: __table({
    name: 'event',
    indexes: [
      { accessor: 'seq', name: 'event_seq_idx_btree', algorithm: 'btree', columns: [
        'seq',
      ] },
    ],
    constraints: [
      { name: 'event_seq_key', constraint: 'unique', columns: ['seq'] },
    ],
  }, EventRow),
  jepaTick: __table({
    name: 'jepa_tick',
    indexes: [
      { accessor: 'slot', name: 'jepa_tick_slot_idx_btree', algorithm: 'btree', columns: [
        'slot',
      ] },
    ],
    constraints: [
      { name: 'jepa_tick_slot_key', constraint: 'unique', columns: ['slot'] },
    ],
  }, JepaTickRow),
  viewer: __table({
    name: 'viewer',
...
const reducersSchema = __reducers(
  ...
  __reducerSchema("configure_jepa", ConfigureJepaReducer),
  __reducerSchema("join", JoinReducer),
  __reducerSchema("observe", ObserveReducer),
  __reducerSchema("populate", PopulateReducer),
