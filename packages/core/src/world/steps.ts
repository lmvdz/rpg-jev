 ...
 * client's keys, by a clicked path, by the path finder, and by the host's
 * `terrainAllows`, so they can never disagree.
 *
 * It is a stand-in for the world's move process (spikes/vocabulary, X1),
 * which will decide this from mass, footing and slope. For now: the ground
 * must be there, dry and not too tall a step, nothing solid may stand on it,
 * and a diagonal step may not cut across a corner it could not walk.
 */

import { LEVEL, SHAPE, type TileGrid } from "./grid.ts";
