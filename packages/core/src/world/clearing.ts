 ...
 * so the renderer meets the real world through the same door. It is pure and
 * engine-agnostic, so both the renderer's test card and the host share the
 * same grown clearing (rule: unobserved time is code, and only one place
 * grows it).
 */
import { Rng } from "../rng.ts";
import { glyphOfChar, glyphOfExtra } from "./glyph-ids.ts";
import { DIR_X, DIR_Z, SHAPE, type Shape, TileGrid } from "./grid.ts";
import { INK } from "./ink.ts";
import { kindIndex } from "./kinds.ts";
import { makeNoise } from "./noise.ts";
import type { ElementLook, ThingView, VisibleStates } from "./scene-thing.ts";
