...
const SOAK = new Kernel(SOAK_DERIVED);
const SOAKS = SOAK_RULES.map((rule) => ready(SOAK, rule));
const WETS = SOAK.test(SOAK_WETS.when);
const COAT = new Kernel(COAT_DERIVED);
const COATS = COAT_RULES.map((rule) => ready(COAT, rule));
