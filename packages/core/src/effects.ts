...
export function validateEffect(world: World, candidate: unknown): string[] {
  ...
  switch (effect.kind) {
    ...
    case "create_debt":
      ...
      break;
    case "settle_debt":
      if (world.debts[effect.id]?.status !== "pending")
        errors.push(`debt ${effect.id} is not pending`);
      break;
    case "transfer":
    ...
    case "say": {
      ...
      if (
        effect.kind === "queue_speech" &&
        world.conversation.queue.some((q) => q.id === intent.id)
      )
