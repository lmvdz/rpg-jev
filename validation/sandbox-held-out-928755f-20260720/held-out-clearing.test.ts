 ...
 * Archived normal-assertion suite for sandbox 928755f. Install beside the matter tests.
 * See the sibling REPORT.md for scope, results, and the one sensing-related adaptation.
 */
...
describe("held-out clearing: exposure and state chains", () => {
  ...
  it("oil is not treated as extinguishing water on burning fuel", () => {
    const world = clearing();
  ...
  it("zero washing liquid cannot remove an existing oil coating", () => {
    const world = clearing();
    put(world, "wash", "soap");
    put(world, "tool", "metal", {
    ...
    // Newer resolve also updates observers. Test physical state, not incidental awareness.
    expect(after.things).toEqual(world.things);
  });
  ...
  // H3: the poc source-side exchange ignored negative transfer; sandbox fixes it.
  it("a cold source gains the heat lost by an equal-capacity hot target", () => {
    const world = clearing();
  ...
  it("waiting past coating exhaustion has the same qualitative result in smaller steps", () => {
    const world = clearing();
    put(world, "tool", "metal", {
      coating: { element: "oil", amount: 0.2, coverage: 1, bond: 0 },
