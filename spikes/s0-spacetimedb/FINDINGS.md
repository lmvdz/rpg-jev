...
3. **Reducers must check who calls them.** Any anonymous client can call any reducer. The commit reducer of the Jev worker, and every reducer that is not a player intent, needs an allow-list on `ctx.sender`. Rule 1 ("only code writes world state") holds at the table level out of the box: anonymous SQL writes are refused and private tables are invisible.
...
| --- | --- | --- | --- | --- |
| 1 | One write path, one event log | Tables written only through reducers; an append-only `event_log`; anonymous writes attempted | Anonymous SQL `INSERT` refused ("not authorized to run SQL DML"). Private tables not visible to clients. Scheduled reducer not callable by a client (HTTP 404). But an anonymous HTTP call to an ordinary reducer succeeded (HTTP 200) | **Go**, with condition 3 |
...
**Verified by measurement here** (local standalone 2.10.1, Windows, one machine, loopback): every number in the checklist and the detail sections; that anonymous SQL writes are refused; that an anonymous client can call an ordinary reducer; that pending fuses survive a stop and fire on restart; that a throwing fuse is consumed; that deleted rows' space is reused and not returned; that the belief query is right at every size; that a TypeScript procedure runs.

**Measured once, or noisy:** tail latencies on a shared machine; restart time (6.0, 9.5 and 16.7 s for logs of similar size); the crossing of a row into and out of a subscription window (once each); the scheduled reducer's refusal of a client call (one HTTP attempt; the 404 said "No such procedure", but a wrong argument encoding was not ruled out); the 15.6 ms stall (one 300-call probe, not kept as a script).
...
- **Section 4, Concurrency.** Add: every reducer that is not a player intent checks `ctx.sender` against an allow-list of worker identities. Add: a decision's hard precondition is a version on the rows it read, compared at commit; this is what S0 built.
