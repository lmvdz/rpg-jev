...
For every scenario, re-express each step using ONLY ids from VOCABULARY.md (P, form tags, S, B, R, X, E, place fields, Load, pick_action). Work from the scenario's `outcome` and `steps`, and ask: would an engine that knows only this vocabulary, plus an element row for each thing, arrive at this outcome?
...

{
  ...
  "steps": [ { "process": "X3", "uses": ["P6", "S1", "R1", "R6"], "effects": ["E1:S3", "E9", "E10"] } ],
